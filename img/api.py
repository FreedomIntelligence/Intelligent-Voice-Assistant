...
def load_audio(reference_audio, sr):
    ...
    waveform, original_sr = torchaudio.load(
        ...
        reference_audio, backend="ffmpeg"
    )
    ...
    if idstr is not None:
        ...
        prompt_tokens = [
            encode_reference(
                ...
                reference_audio=audio_to_bytes(ref_audio),
        ...
        if refs is None:
            refs = []
        print('这个是refs：' + str(refs))
        prompt_tokens = [
            encode_reference(
                decoder_model=decoder_model,
                reference_audio=audio_to_bytes(ref.audio),
                enable_reference_audio=True,
            )
            for ref in refs
        ]
...
pip3 install torch==2.3.0 torchaudio==2.3.0

export CUDA_VISIBLE_DEVICES=1
'''
# ps -ef | grep api | grep -v grep | awk '{print "kill "$2}' | sh
